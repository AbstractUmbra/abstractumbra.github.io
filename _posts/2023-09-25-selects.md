...
> At the time of writing, only a [string select](#select) is supported.

This can be useful for gathering user input in a more interactive way. Here's an example of how to use a select menu in a modal.
...
- The `disabled` kwarg for selects is not valid and will raise an error if used inside a modal.
...
class ModalWithSelect(ui.Modal):
    ...
    fruit_select = ui.Label(
        ...
        # this is where the select (or TextInput) component goes
        component=ui.Select(
    ...
    reason = ui.Label(
        label="Why is that your favorite fruit?",
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        ...
        chosen_fruit = self.fruit_select.component.value
